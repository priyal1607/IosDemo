...
class GallerySeeAllDelegate : NSObject {
    ...
    fileprivate func registerCell() {
        let cellTypes: [UICollectionViewCell.Type] = [GallerySeeAllCollViewCell.self]
...
extension GallerySeeAllDelegate: UICollectionViewDelegate {
...
extension GallerySeeAllDelegate: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(with: GallerySeeAllCollViewCell.self, for: indexPath)
        
        cell.configureGalleryDetailsCell(data: arrSource[indexPath.row], imgUrl: imgURL)
        
        return cell
    }
}

extension GallerySeeAllDelegate: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: IS_IPAD ? (SCREEN_WIDTH - (IPAD_MARGIN)) : ((SCREEN_WIDTH/2)-20) , height: ((SCREEN_WIDTH/2)-32)/1.21)
    }
